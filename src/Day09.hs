...
day9a :: String -> Int
day9a input = sum $ map (\p -> 1 + tileMap M.! p) lowPoints
  where
    tileMap = parse input
...
day9b =
  product .
  map S.size .
  take 3 . sortBy (\a b -> S.size b `compare` S.size a) . findBasins . parse
...
findLowPoints :: M.Map Position Int -> [Position]
...
findBasins tileMap =
  let lowPoints = findLowPoints tileMap
      basins =
